...
def run():
    ...
    oskar_path = get_pwd(__file__) / 'test/oskar'
    ...
    shutil.copy(get_oskar_data('oskar_sim_interferometer.ini'), interferometer)
    shutil.copy(get_oskar_data('sky.osm'), oskar_path / 'sky.osm')
    shutil.copytree(get_oskar_data('telescope.tm'), oskar_path / 'telescope.tm', dirs_exist_ok=True)
    imager = oskar_path / 'oskar_imager.ini'
    shutil.copy(get_oskar_data('oskar_imager.ini'), imager)
